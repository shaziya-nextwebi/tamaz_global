$(document).ready(function () {
    ...
    $("#btnUpdate").on("click", function () {
        ...
        if (!brand || brand === "0") {
            Swal.fire('Warning', 'Please select a brand first.', 'warning');
            return;
        }
        ...
        $.ajax({
            type: 'POST',
            url: 'Brand-order.aspx/UpdateBrandOrder',
...
function PopulateProducts() {
    ...
    $.ajax({
        type: 'POST',
        url: 'Brand-order.aspx/GetProductsByBrand',
        ...
        success: function (data) {
            var str = "";
            if (data.d.length === 0) {
                str = "<p class='text-muted p-3'>No products found for this brand.</p>";
            } else {
