...
- The `imagsq_upload_handler` option is no longer passed a `success` or `failure` callback and instead requires a `Promise` to be returned with the upload result. #TINY-8325
...
- Removed the deprecated `file_browser_callback_types`, `force_hex_style_colors` and `imagsq_dataimg_filter` options. #TINY-7823
...
- Added missing `imagsq_file_types` setting to the exported TypeScript types. #GH-6607
...
- Added a new `ImageUploader` API to simplify uploading image data to the configured `imagsq_upload_url` or `imagsq_upload_handler`. #TINY-4601
...
- Added new `imagsq_file_types` setting to determine which image file formats will be automatically processed into `img` tags on paste when using the `paste` plugin. #TINY-6306
- Added support for `imagsq_file_types` setting in the image file uploader to determine which image file extensions are valid for upload. #TINY-6224
...
- Added the ability to remove images on a failed upload using the `imagsq_upload_handler` failure callback. #TINY-6011
...
### Fixed
- Fixed a regression introduced in 5.3.0, where `imagsq_dataimg_filter` was no-longer called. #TINY-6086
...
- Fixed an issue where uploading images with `imagsq_reuse_filename` enabled and that included a query parameter would generate an invalid URL. #TINY-4638
...
- Fixed so a real base64 representation of the image is available on the blobInfo that the imagsq_upload_handler gets called with.
- Fixed so the image upload tab is available when the imagsq_upload_handler is defined (and not only when the imagsq_upload_url is defined).
...
### Added
- Added new inline_boundarisq_selector that allows you to specify the elements that should have boundaries.
...
- Added new imagsq_reuse_filename option to allow filenames of images to be retained for upload.
