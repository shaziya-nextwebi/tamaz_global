...
public partial class BrandPage : System.Web.UI.Page
{
    ...
    public string strBrandUrl = "";
    public int currentPage = 1;
    public int totalProducts = 0;

    protected void Page_Load(object sender, EventArgs e)
    {
        // Read brandurl from route
        string brandUrl = Page.RouteData.Values["brandurl"] != null
                          ? Page.RouteData.Values["brandurl"].ToString()
                          : "";
        ...
        if (!IsPostBack)
        {
            LoadBrandDetails(brandUrl);
            LoadProducts(brandUrl, currentPage);
        }
    }

    public void LoadBrandDetails(string brandUrl)
    {
        try
        {
            SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["conT"].ConnectionString);
            List<Brand> brands = Brand.GetAllBrand(con);
            Brand brand = brands.FirstOrDefault(x => x.BrandUrl.ToLower() == brandUrl.ToLower());
            ...
            strPageTitle = brand.PageTitle != "" ? brand.PageTitle : brand.BrandName + " - TAMAZ Global";
            strMetaDesc = brand.MetaDesc;
            strMetaKeys = brand.MetaKeys;
            strFullDesc = brand.FullDesc;
            strShortDesc = brand.ShortDesc != "" ? brand.ShortDesc : "Browse our premium collection.";

            // Breadcrumb
            strBreadcrumb = "<a href='/' class='hover:text-[#B91C1C] transition-colors'>Home</a>" +
                            "<span class='mx-2'>/</span>" +
                            "<a href='/Brand' class='hover:text-[#B91C1C] transition-colors'>Brand</a>" +
                            "<span class='mx-2'>/</span>" +
                            "<span class='text-[#0F172A] font-medium'>" + brand.BrandName + "</span>";
        }
        catch (Exception ex)
        {
            ExceptionCapture.CaptureException(HttpContext.Current.Request.Url.PathAndQuery, "LoadBrandDetails", ex.Message);
        }
    }

    public void LoadProducts(string brandUrl, int pageNo)
    {
        try
        {
            SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["conT"].ConnectionString);
            List<ProductDetails> products = ProductDetails.GetAllProductsBasedOnBrand(con, brandUrl, pageNo);

            strProducts = "";

            if (products.Count == 0)
            {
                strProducts = "<p class='text-gray-400 col-span-4 text-center py-10'>No products found for this brand.</p>";
                return;
            }
            ...
            // Pagination
            BuildPagination(brandUrl, pageNo, totalProducts);
        }
        catch (Exception ex)
        {
    ...
    public void BuildPagination(string brandUrl, int currentPg, int total)
    {
        ...
        string baseUrl = "/Brand/" + brandUrl + "?page=";
        strPagination = "";
